...
public class StorableEngine {
    ...
    public static <T> void read(@Nonnull Registry registry, @Nonnull Set<StoreFor> phase, @Nonnull NBTTagCompound tag,
        @Nonnull T object) throws IllegalAccessException, InstantiationException, NoHandlerFoundException {
        INSTANCE.get()
            .read_impl(registry, phase, tag, object);
    }

    public static <T> void store(@Nonnull Registry registry, @Nonnull Set<StoreFor> phase, @Nonnull NBTTagCompound tag,
        @Nonnull T object) throws IllegalAccessException, InstantiationException, NoHandlerFoundException {
        INSTANCE.get()
            .store_impl(registry, phase, tag, object);
    }

    public <T> void read_impl(@Nonnull Registry registry, @Nonnull Set<StoreFor> phase, @Nonnull NBTTagCompound tag,
        @Nonnull T object) throws IllegalAccessException, InstantiationException, NoHandlerFoundException {
    ...
    public <T> void store_impl(@Nonnull Registry registry, @Nonnull Set<StoreFor> phase, @Nonnull NBTTagCompound tag,
        @Nonnull T object) throws IllegalAccessException, InstantiationException, NoHandlerFoundException {
        Class<? extends Object> clazz = object.getClass();
        if (!fieldCache.containsKey(clazz)) {
            cacheHandlers(registry, clazz);
    ...
    private void cacheHandlers(@Nonnull Registry registry, Class<?> clazz)
            ...
            if (annotation != null) {
                if (annotation.handler() == HandleStorable.class) {
                    ...
                    if (!superclassCache.containsKey(superclazz)) {
                        superclassHandlerCache.put(
                            superclazz,
                            (List<IHandler>) Arrays.asList(
                                annotation.handler()
                                    .newInstance()));
                    }
                }
