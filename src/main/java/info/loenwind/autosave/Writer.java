...
public class Writer {
     ...
     */
    public static <T> void write(@Nonnull Registry registry, @Nonnull Set<Store.StoreFor> phase,
        @Nonnull NBTTagCompound tag, @Nonnull T object) {
        try {
    ...
    public static <T> void write(@Nullable Set<Store.StoreFor> phase, @Nullable NBTTagCompound tag, @Nonnull T object) {
        write(
            ...
            NullHelper.notnull(phase, "Missing phase"),
            NullHelper.notnull(tag, "Missing NBT"),
            object);
    }
     ...
     *                 The object that should be stored
     */
    public static <T> void write(@Nonnull Registry registry, @Nonnull StoreFor phase, @Nullable NBTTagCompound tag,
        @Nonnull T object) {
        write(
            registry,
            NullHelper.notnullJ(EnumSet.of(phase), "EnumSet.of()"),
            NullHelper.notnull(tag, "Missing NBT"),
            object);
    }
    ...
    public static <T> void write(@Nonnull Registry registry, @Nullable NBTTagCompound tag, @Nonnull T object) {
        write(
            registry,
            NullHelper.notnullJ(EnumSet.allOf(StoreFor.class), "EnumSet.allOf()"),
            NullHelper.notnull(tag, "Missing NBT"),
            object);
    }
