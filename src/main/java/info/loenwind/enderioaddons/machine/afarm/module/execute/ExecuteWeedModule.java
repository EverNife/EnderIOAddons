        ...
        if (workTile.doWeed) {
            if (workTile.farm.canUsePower(farmRFperRaking.getInt())) {
                ...
                if (rakeSlot != -1) {
                    ...
                    boolean isIndustrialRake = stack.getItem() == ItemMachinePart.itemMachinePart
                        && stack.getItemDamage() == MachinePart.IRAKE.ordinal();
                    ...
                    if (isIndustrialRake
                        ? workTile.agricraft.removeWeeds(world, workTile.bc.x, workTile.bc.y, workTile.bc.z, false)
                        : workTile.agricraft.removeWeeds(world, workTile.bc.x, workTile.bc.y, workTile.bc.z, false)) {
                        workTile.farm
                            .usePower(isIndustrialRake ? farmRFperRakingIndustrial.getInt() : farmRFperRaking.getInt());
