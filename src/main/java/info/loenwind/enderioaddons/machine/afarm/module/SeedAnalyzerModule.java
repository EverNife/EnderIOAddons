...
public class SeedAnalyzerModule implements IAfarmControlModule {
    ...
    static boolean isSameSeed(ItemStack seed1, ItemStack seed2) {
        if (seed1.getItem() == seed2.getItem()) {
            if (seed1.getItemDamage() != OreDictionary.WILDCARD_VALUE
                && seed2.getItemDamage() != OreDictionary.WILDCARD_VALUE
                && seed1.getItemDamage() != seed2.getItemDamage()) {
                return false;
            } else {
                if (seed1.stackTagCompound == seed2.stackTagCompound || seed1.stackTagCompound == null
                    || seed2.stackTagCompound == null
                    || ItemStack.areItemStackTagsEqual(seed1, seed2)) {
                    return true;
                }
