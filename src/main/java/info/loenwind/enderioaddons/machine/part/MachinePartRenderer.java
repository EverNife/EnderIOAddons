...
public class MachinePartRenderer implements IItemRenderer {
    ...
    public void renderItem(ItemRenderType type, ItemStack item, Object... data) {
        @Nonnull
        final RenderBlocks renderBlocks = notnullM(
            (RenderBlocks) data[0],
            "Game state error: Missing RenderBlock parameter");
        @Nonnull
    ...
    private static interface Impl {
        ...
        void render(@Nonnull ItemStack item, @Nonnull RenderBlocks renderBlocks,
            RendererFrameworkMachine frameRenderer);
    }
    ...
    private static class ImplTechne implements Impl {
        ...
        public void setup_Entity(ItemRenderType type, ItemStack item, RenderBlocks renderBlocks) {
            ...
            if (rotationInItemframesEnabled.getBoolean() && item.isOnItemFrame()) {
                TickRotator.getInstance()
                    .rotateGl(2);
            }
            GL11.glScalef(0.5F, 0.5F, 0.5F);
            if (!MachinePart.values()[item.getItemDamage()].hasFrame
                && !MachinePart.values()[item.getItemDamage()].hasTanks) {
        ...
        public void render(ItemStack item, RenderBlocks renderBlocks, RendererFrameworkMachine frameRenderer) {
            ...
            frameRenderer
                .renderInventoryBlock(AbstractBlockFramework.blockDummy, item.getItemDamage() + 16, 0, renderBlocks);
    ...
    private static class ImplBlockAsItem implements Impl {
        ...
        public void render(ItemStack item, RenderBlocks renderBlocks, RendererFrameworkMachine frameRenderer) {
            GL11.glEnable(GL11.GL_ALPHA_TEST);
            renderBlocks
                .setOverrideBlockTexture(ItemMachinePart.itemMachinePart.getIconFromDamage(item.getItemDamage()));
