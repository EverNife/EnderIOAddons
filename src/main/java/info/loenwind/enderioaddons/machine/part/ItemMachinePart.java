...
public class ItemMachinePart extends Item {
    ...
    public static final ModObject ModObject_itemMachinePart = EnumHelper
        .addEnum(ModObject.class, "itemMachineParts", new Class<?>[0], new Object[0]);
    ...
    public void getSubItems(Item par1, CreativeTabs par2CreativeTabs, List par3List) {
        for (int j = 0; j < MachinePart.values().length; ++j) {
            if ((AgriDetector.hasAgri && j == MachinePart.AFARMINFO.ordinal())
                || (!AgriDetector.hasAgri && MachinePart.isAgri(j))) {
                continue;
            }
        ...
        if (stack.getItemDamage() == MachinePart.COOKIE.ordinal()) {
            if (!world.isRemote) {
                ...
                for (int i = 0; i < player.inventory.mainInventory.length; i++) {
                    ...
                    if (ItemUtil.areStackMergable(inventoryItem, strip)
