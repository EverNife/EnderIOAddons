...
public class EngineTcom {
    ...
    public boolean add(ItemStack itemStack) {
        ...
        for (int i = 0; i < stackSize; i++) {
            factor += (maxDamage - itemDamage) / (float) maxDamage
                * (1f - minloss - (float) rand.nextDouble() * randloss);
        }
        for (Mats mat : mats) {
    ...
    private EnchantmentData getEnchantmentData(int id, int enchantmentPower) {
        ...
        final int maxLevel = (int) (enchantment.getMaxLevel() < enchantmentPool[id] ? enchantment.getMaxLevel()
            : enchantmentPool[id]);
        int level = 1;
                    ...
                    if (enchantmentInPool.canApply(itemStack)) {
                        if (enchantmentsOnItemStack.isEmpty()) {
                            ...
                            for (Integer id2 : enchantmentsOnItemStack.keySet()) {
                                ...
                                if (!enchantmentInPool.canApplyTogether(enchantmentOnItemStack)
    ...
    public boolean addEnchantment(ItemStack itemStack, int id, int enchantmentPower) {
        ...
        if (itemStack == null || itemStack.getItem() == null
            ...
            || enchantmentInPool == null
            || enchantmentPool[id] < 1) {
            return false;
        }
        if (itemStack.getItem() == Items.book) {
            if (!enchantmentInPool.isAllowedOnBooks()) {
