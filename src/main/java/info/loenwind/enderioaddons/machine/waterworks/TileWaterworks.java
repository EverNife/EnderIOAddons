...
public class TileWaterworks extends TileEnderIOAddons implements IFrameworkMachine, IFluidHandler, ITankAccess {
    ...
    public Fluid getTankFluid(@Nonnull TankSlot tankSlot) {
        switch (tankSlot) {
            case FRONT_LEFT:
                return inputTank.getFluidAmount() > 0 ? inputTank.getFluid()
                    .getFluid() : null;
            case BACK_LEFT:
                return inputTank.getFilledRatio() > .34 ? inputTank.getFluid()
                    .getFluid() : null;
            case BACK_RIGHT:
                return inputTank.getFilledRatio() > .67 ? inputTank.getFluid()
                    .getFluid() : null;
            case FRONT_RIGHT:
                return outputTank.getFluidAmount() > 0 ? outputTank.getFluid()
                    .getFluid() : null;
        }
        return null;
    }
    ...
    public FluidStack drain(@Nullable ForgeDirection from, @Nullable FluidStack resource, boolean doDrain) {
        if (from == null || isSideDisabled(from.ordinal())
            ...
            || resource == null
            || !resource.isFluidEqual(outputTank.getFluid())) {
            return null;
        }
    ...
    public boolean canFill(@Nullable ForgeDirection from, @Nullable Fluid fluid) {
        ...
        if (outputTank.getFluidAmount() == 0) {
            ...
            return data.isMatchingInputForOutput(
                fluid,
                outputTank.getFluid()
                    .getFluid());
        }
    ...
    protected boolean canInsertResult(float chance, @Nullable IMachineRecipe nextRecipe) {
        ...
        progress_in = inputTank.getFluid()
            .getFluid();

        int level = data.getLevelFromInput(
            inputTank.getFluid()
                .getFluid());
        if (level < 0) {
            return false;
        ...
        if (!data.isMatchingOutputForInput(
            progress_in,
            outputTank.getFluidAmount() > 0 ? outputTank.getFluid()
                .getFluid() : null)) {
            return false;
        }
    ...
    protected void taskComplete() {
        if (currentTask != null) {
            ...
            CreationResult creationResult = engine
                .createItems(stash, level, this, slotDefinition.minOutputSlot, slotDefinition.maxOutputSlot, true);
            ...
            if (creationResult == CreationResult.OK || creationResult == CreationResult.NO_INPUTS) {
                ...
                if (amount > 0) {
                    ...
                    } else if (progress_out2 == null || outputTank.getFluid()
                        .getFluidID() == progress_out2.getID()) {
