...
public final class FluidHelper {
    ...
    private FluidHelper(@Nonnull World world, @Nonnull FluidStack stack, @Nullable BlockCoord startbc)
        throws Exception {
        this.world = world;
        this.stack = stack;
    ...
    @Nonnull
    private static final ForgeDirection[] DIRECTIONS_INIT = { ForgeDirection.UP, ForgeDirection.NORTH,
        ForgeDirection.SOUTH, ForgeDirection.WEST, ForgeDirection.EAST };

    @Nullable
    public static FluidHelper getInstance(@Nonnull World world, @Nonnull BlockCoord bc, @Nonnull FluidFilter whitelist,
        @Nonnull FluidFilter blacklist) {
    ...
    public static boolean isSameLiquid(@Nonnull FluidStack fs, @Nonnull World world, @Nonnull BlockCoord bc) {
        return world.blockExists(bc.x, bc.y, bc.z) && bc.getBlock(world) == fs.getFluid()
            .getBlock();
    }
    ...
    public boolean isFlowingVertically2(@Nonnull BlockCoord bc) {
        ...
        return isSameLiquid(getLocation(bc, ForgeDirection.UP)) && isSameLiquid(getLocation(bc, ForgeDirection.DOWN))
            && !isSourceBlock(downflow);
    }
    ...
    public void doFlow(@Nonnull BlockCoord bc0, @Nonnull BlockCoord bc1) {
        if (isInWorld(bc1)) {
            world.setBlock(
                ...
                bc1.y,
                bc1.z,
                world.getBlock(bc0.x, bc0.y, bc0.z),
                world.getBlockMetadata(bc0.x, bc0.y, bc0.z),
                3);
        }
        switch (type) {
            case FINITE:
    ...
    @Nonnull
    public static final ForgeDirection[] DIRECTIONS = { ForgeDirection.NORTH, ForgeDirection.SOUTH, ForgeDirection.WEST,
        ForgeDirection.EAST };
    ...
    public ReturnObject eatOrPullFluid(@Nonnull BlockCoord bc) {
        ...
        if (isSameLiquid(bc)) {
            ...
            if (!result.inProgress && result.result == null) {
                ...
                // may be messed up and give us some fake flowing blocks. Try to remedy this be forcing it to re-flow
                // them.
                for (BlockCoord blockCoord : seen) {
                    blockCoord = notnull(
                        blockCoord,
                        "FluidHelper is confused: I remember to have visited an invalid location?");
                ...
                if (isSameLiquid(bc2)) {
                    if (isSourceBlock(bc2)) {
                        if (foundStepUp) { // don't flow unless there is a "down" to flow to
                            if (isSameLiquid(getLocation(bc2, downflowDirection))
                                && !isSourceBlock(getLocation(bc2, downflowDirection))
                                && isSameLiquid(getLocation(bc, downflowDirection))
                                && !isSourceBlock(getLocation(bc, downflowDirection))) {
                                // if we can drop the source block down by one without disconnecting it from us, we do
                                // so
