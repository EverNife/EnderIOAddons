...
@Storable
public class TileDrain extends TileEnderIOAddons
    implements IFluidHandler, IWaterSensitive, IDrainingCallback, ITankAccess {
        ...
        if (shouldDoWorkThisTick(modulo) && tank.getAvailableSpace() >= ONE_BLOCK_OF_LIQUID) {
            ...
            if (isFoodDrain()) {
                if (Config.foodDrainCollectsFlowingMilk.getBoolean()) {
                    if (tankfluid != null) {
                        instance = FluidHelper.getInstance(
                            notnull(worldObj, "Invalid game state: World is missing"),
            ...
            if (instance != null) {
                ...
                if (Config.foodDrainCollectsMilkFromCows.getBoolean() && isFoodDrain()
                    && (tankfluid == null || tankfluid.getFluid() == Fluids.MILK.getFluid())) {
                    AxisAlignedBB bb = getBlockType().getCollisionBoundingBoxFromPool(worldObj, xCoord, yCoord, zCoord)
                        .expand(Config.foodDrainRange.getFloat(), 1.5f, Config.foodDrainRange.getFloat());
                ...
                if (dryruncount++ > 60) {
                    ...
                    nowater.clear();
                    if (registered) {
                        InfiniteWaterSourceStopper.getInstance()
                            .unregister(notnull(worldObj, "Invalid game state: World is missing"), this);
                        registered = false;
                    }
                }
            }
    ...
    public int getComparatorOutput() {
        ...
        return info == null || info.fluid == null ? 0
            : (int) (((double) info.fluid.amount / (double) info.capacity) * 15);
    }
    ...
    public void setWorldObj(@Nullable World p_145834_1_) {
        ...
        if (!worldObj.isRemote && !nowater.isEmpty() && !registered) {
            ...
            InfiniteWaterSourceStopper.getInstance()
                .register(notnull(worldObj, "Invalid game state: World is missing"), this);
            registered = true;
        }
    }
