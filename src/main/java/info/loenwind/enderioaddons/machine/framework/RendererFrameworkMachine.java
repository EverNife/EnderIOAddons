...
public class RendererFrameworkMachine extends TechneMachineRenderer<AbstractTileFramework> {
    ...
    private static void renderSubBlock(int x, int y, int z, @Nonnull AbstractMachineEntity te,
        @Nonnull TankSlot tankSlot) {
        BoundingBox bb = BoundingBox.UNIT_CUBE;

        int[] pos = translateToSlotPosition(
    ...
    public static IIcon[] getBlockTextures(@Nonnull AbstractMachineEntity te, @Nonnull TankSlot tankSlot) {
        ...
        for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
            if (te.isActive()) {
                icons[i++] = fm
                    .getSlotIcon(tankSlot, ClientProxy.sideAndFacingToSpriteOffset[dir.ordinal()][facing] + 6);
            } else {
