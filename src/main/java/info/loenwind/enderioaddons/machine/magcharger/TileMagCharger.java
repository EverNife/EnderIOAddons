...
public class TileMagCharger extends AbstractTileFramework implements INetworkUpdatable {
    ...
    protected boolean isMachineItemValidForSlot(int i, ItemStack itemstack) {
        if (slotDefinition.isInputSlot(i)) {
            return itemstack != null && itemstack.getItem() != null
                && itemstack.getItem() instanceof ItemFlag
    ...
    private boolean outputSlotsIsNotFull() {
        return inventory[slotDefinition.getMinOutputSlot()].stackSize
            < inventory[slotDefinition.getMinOutputSlot()].getMaxStackSize();
    }
    ...
    public float getProgressScaled() {
        if (progress <= 0) {
            ...
            return ((magcMagnetizingTicksPerItem.getInt() + 1) - progress)
                / (magcMagnetizingTicksPerItem.getFloat() + 1f);
        }
    }
