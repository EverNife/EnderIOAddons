...
public class TileNiard extends TileEnderIOAddons implements IFluidHandler, ITankAccess {
        ...
        if (tank.getFluidAmount() < tank.getCapacity()) {
            ...
            if (target != null) {
                ...
                if (tank.getFluidAmount() > 0) {
                    FluidStack canPull = tank.getFluid()
                        .copy();
    ...
    protected boolean canTick(boolean redstoneChecksPassed) {
        if (redstoneChecksPassed) {
            ...
            if (tank.getAvailableSpace() > 0 && inventory[getSlotDefinition().getMinInputSlot()] != null
                && shouldDoWorkThisTick(20)) {
                drainFullContainer();
    ...
    protected boolean doTick() {
        if (sleep == 0) {
            ...
            if (shouldDoWorkThisTick(modulo) && tank.getFluidAmount() > 0) {
                if (tank.getFluid()
                    .getFluid() == EnderIO.fluidXpJuice) {
                    int amount = tank.getFluidAmount();
                    boolean looping = true;
                    while (looping) {
                        int remaining = engine.setRadius(range)
                            .work(amount);
                ...
                } else if (tank.getFluidAmount() >= ONE_BLOCK_OF_LIQUID && engine.setFluid(
                    tank.getFluid()
                        .getFluid())
                    .setRadius(range)
                    .work()) {
