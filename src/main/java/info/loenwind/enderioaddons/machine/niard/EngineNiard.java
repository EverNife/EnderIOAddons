...
public class EngineNiard {
    ...
    public boolean work() {
        ...
        for (int i = 0; i < radiusItr.size(); i++) {
            ...
            while (isInWorld(next) && (owner.getWorldObj()
                .isAirBlock(next.x, next.y, next.z) || (isSameLiquid(next) && isFlowingBlock(next)))) {
                seen.add(next);
            ...
            if (!seen.isEmpty()) {
                ...
                owner.getWorldObj()
                    .notifyBlockChange(base.x, base.y, base.z, base.getBlock(owner.getWorldObj()));
                return true;
            }
        }
        return false;
    ...
    public int work(int xp_in_mb) {
        ...
        if (radius >= 0) {
            for (int i = 0; i < radiusItr.size(); i++) {
                BlockCoord next = radiusItr.next()
                    .getLocation(ForgeDirection.DOWN);
                if (isInWorld(next) && owner.getWorldObj()
                    .isAirBlock(next.x, next.y, next.z)) {
    ...
    private boolean isInWorld(BlockCoord bc) {
        return owner.getWorldObj()
            .blockExists(bc.x, bc.y, bc.z);
    }

    private boolean isSameLiquid(@Nonnull BlockCoord bc) {
        final Block wblock = bc.getBlock(owner.getWorldObj());
        return wblock == block || (block == Blocks.water && wblock == Blocks.flowing_water)
            || (block == Blocks.lava && wblock == Blocks.flowing_lava);
    }
    ...
    private boolean isFlowingBlock(@Nonnull BlockCoord bc) {
        switch (type) {
            ...
            case VANILLA:
                return owner.getWorldObj()
                    .getBlockMetadata(bc.x, bc.y, bc.z) != 0;
        }
                ...
                if (world.provider.isHellWorld && fluid == FluidRegistry.WATER && !niardAllowWaterInHell.getBoolean()) {
                    ...
                    for (int l = 0; l < 8; ++l) {
                        spawnParticle(
                            world,
                            "largesmoke",
                            bc.x - 1 + 3 * Math.random(),
                            bc.y + Math.random(),
                            bc.z - 1 + 3 * Math.random(),
                            0.0D,
                            0.0D,
                            0.0D);
                    }
    ...
    private void setVerticalBlock(BlockCoord bc, boolean blockUpdate) {
        ...
        owner.getWorldObj()
            .setBlock(bc.x, bc.y, bc.z, blockToSet, metaToSet, blockUpdate ? 3 : 2);
    }
