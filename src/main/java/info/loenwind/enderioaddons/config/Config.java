...
public enum Config {
    ...
    drainPerBucketEnergyUseRF(Section.DRAIN, 400,
    ...
    drainAllowOnDedicatedServer(Section.DRAIN, false,
        "Allow the use of the drain on a dedicated server. This does not effect the food drain", true, true), //
    ...
    waterWorksWaterReductionPercentage(Section.WATERWORKS, 10,
        "Amount of liquid produced when processing in percent (0-100) of the input liquid", true, false), //

    waterWorksRFperTask1(Section.WATERWORKS, 5 * 20 * 20 * 1.0d, "RF per task (machine with no capacitor upgrades)",
        true, false), //
    waterWorksRFperTask2(Section.WATERWORKS, 5 * 20 * 20 * 1.75f * 0.9d,
        "RF per task (machine with first capacitor upgrade)", true, false), //
    waterWorksRFperTask3(Section.WATERWORKS, 5 * 20 * 20 * 3 * 0.8d,
        "RF per task (machine with second capacitor upgrade)", true, false), //
    ...
    waterWorksRFusePerTick1(Section.WATERWORKS, 20,
        "RF a machine can use per tick (machine with no capacitor upgrades)", true, false), //
    waterWorksRFusePerTick2(Section.WATERWORKS, 40,
        "RF a machine can use per tick (machine with first capacitor upgrade)", true, false), //
    waterWorksRFusePerTick3(Section.WATERWORKS, 60,
        "RF a machine can use per tick (machine with second capacitor upgrade)", true, false), //

    waterWorksRFinPerTick1(Section.WATERWORKS, 100,
        "RF a machine can accept per tick (machine with no capacitor upgrades)", true, false), //
    waterWorksRFinPerTick2(Section.WATERWORKS, 200,
        "RF a machine can accept per tick (machine with first capacitor upgrade)", true, false), //
    waterWorksRFinPerTick3(Section.WATERWORKS, 800,
        "RF a machine can accept per tick (machine with second capacitor upgrade)", true, false), //
    ...
    niardPerBucketEnergyUseRF(Section.NIARD, 400,
    ...
    voidTankContinuousEnergyUseRF1(Section.VOIDTANK, 10,
        "The amount of power used by a Portal Tank per tick (machine with no capacitor upgrades)", true, false), //
    voidTankContinuousEnergyUseRF2(Section.VOIDTANK, 100,
        "The amount of power used by a Portal Tank per tick (machine with first capacitor upgrade)", true, false), //
    voidTankContinuousEnergyUseRF3(Section.VOIDTANK, 10000,
        "The amount of power used by a Portal Tank per tick (machine with second capacitor upgrade)", true, false), //

    voidTankEnabled(Section.RECIPES, true, "Enable the crafting recipe for the Portal Tank", true, true), //

    pMonEnableDynamicTextures(Section.PMON, true,
    ...
    tcomRecyclingLossChanced(Section.TCOM, 0.5d, "Chance based loss of recycling in the Loot Recycler (0-1)", true,
        false), //

    tcomRecyclingPowerPerTick(Section.TCOM, 10, "RF/tick for recycling in the Loot Recycler", true, false), //
    tcomRecyclingTicksPerItem(Section.TCOM, 45,
    ...
    magcMagnetizingTicksPerItem(Section.MAGCHARGER, 20,
    ...
    decoBlockInvisibleIsInvisible(Section.DECO, true,
        "Allow the invisible block to be invisible. Otherwise it will mimic a quite clear glass block.", true, false), //
    ...
    farmBreaksWoodenRakesChance(Section.FARM, 1d / 300d,
    ...
    farmRFperSeedHarvest(Section.FARM, 500,
    ...
    seedRecipesEnabled(Section.FARM, true,
        "If disabled there will be no recipe to craft capacitor plant seeds and no recipes to use the fruits of the capacitor plant. "
            + "The seeds may still be generated as random loot by AgriCraft",
        true, true), //
    ...
    plantDropsHighGain(Section.FARM,
        "enderioaddons:itemMachineParts@23=10000, enderioaddons:itemMachineParts@24=3000, enderioaddons:itemMachineParts@25=500, "
    ...
    plantRequiredBlocks(Section.FARM, "EnderIO:blockCapBank@1, minecraft:bedrock@-1, EnderIO:blockDarkIronBars@-1",
        "Three items that define the soil, block under soil and block around for the capacitor plant. Please note that this are item IDs, not block IDs, but block"
            + " meta values. The blocks for these item will be looked up as ususal.",
        true, true), //
    seedsAutomaticHarvestingEnabled(Section.FARM, false,
    ...
    farmDebugLoggingEnabled(Section.FARM, false,
        "When enabled, the farm will write extensive information about its operation into the logfile"), //

    farmEnabled(Section.RECIPES, true,
        "Enable the crafting recipe for the AgriCraft farming station (will only work if AgriCraft is installed)", true,
        true), //

    flowingMilkCuresPoison(Section.LIQUIDS, 0.001, "Chance that flowing milk will cure poison (applies once per tick)"), //
    sourceMilkIsConsumedWhenCuringPoison(Section.LIQUIDS, 0.25,
    ...
    fortuneCookiesCraftingEnabled(Section.RECIPES, true,
        "When enabled, the fortune cookies will have a crafting recipe", true, true), //
    ...
    growthRequirementDebuggingEnabled(Section.DEV, false,
    ...
    void load(Configuration config) {
        ...
        if (defaultValue instanceof Integer) {
            value = setPropertyData(config.get(section.name, name(), (Integer) defaultValue, description))
                .getInt((Integer) defaultValue);
        } else if (defaultValue instanceof Double) {
            value = setPropertyData(config.get(section.name, name(), (Double) defaultValue, description))
                .getDouble((Double) defaultValue);
        } else if (defaultValue instanceof Boolean) {
            value = setPropertyData(config.get(section.name, name(), (Boolean) defaultValue, description))
