...
public class EioaGrowthRequirement implements IGrowthRequirement {
    ...
    public EioaGrowthRequirement() {
        ...
        darkBar = new BlockWithMeta(
            Block.getBlockFromItem(Recipes.darkSteelBars.getItem()),
        ...
        if (list != null) {
            if (list.size() == 3) {
                ...
                if (blockCapBank != null) {
                    if (itemStackCapBank.getItemDamage() <= 15
                        || itemStackCapBank.getItemDamage() == OreDictionary.WILDCARD_VALUE) {
                        capBank = new BlockWithMeta(blockCapBank, itemStackCapBank.getItemDamage(), true);
                    } else {
                        Log.warn(
                            "Config value 'plantRequiredBlocks' is invalid (first element has invalid meta) and will be (partially) ignored.");
                    }
                } else {
                    Log.warn(
                        "Config value 'plantRequiredBlocks' is invalid (first element has no block) and will be (partially) ignored.");
                }
                ...
                if (blockBedrock != null) {
                    if (itemStackBedrock.getItemDamage() <= 15
                        || itemStackBedrock.getItemDamage() == OreDictionary.WILDCARD_VALUE) {
                        bedrock = new BlockWithMeta(blockBedrock, itemStackBedrock.getItemDamage(), true);
                    } else {
                        Log.warn(
                            "Config value 'plantRequiredBlocks' is invalid (second element has invalid meta) and will be (partially) ignored.");
                    }
                } else {
                    Log.warn(
                        "Config value 'plantRequiredBlocks' is invalid (second element has no block) and will be (partially) ignored.");
                }
                ...
                if (blockDarkBar != null) {
                    if (itemStackDarkBar.getItemDamage() <= 15
                        || itemStackDarkBar.getItemDamage() == OreDictionary.WILDCARD_VALUE) {
                        darkBar = new BlockWithMeta(blockDarkBar, itemStackDarkBar.getItemDamage(), true);
                    } else {
                        Log.warn(
                            "Config value 'plantRequiredBlocks' is invalid (third element has invalid meta) and will be (partially) ignored.");
                    }
                } else {
                    Log.warn(
                        "Config value 'plantRequiredBlocks' is invalid (third element has no block) and will be (partially) ignored.");
                }
        ...
        try {
            Class<?> newGrowthRequirementHandler = Class
                .forName("com.InfinityRaider.AgriCraft.farming.growthrequirement.GrowthRequirementHandler");
        ...
        try {
            Class<?> growthRequirementHandler = Class
                .forName("com.InfinityRaider.AgriCraft.farming.GrowthRequirementHandler");
    ...
    public boolean canGrow(World world, int x, int y, int z) {
        if (Config.growthRequirementDebuggingEnabled.getBoolean()) {
            ...
            if (isValidSoil(world, x, y - 1, z)) {
                ...
                txt += "Soil (" + capBank
                    + ") is NOT valid, it is "
                    + WorldHelper.getBlock(world, x, y, z)
                    + ":"
                    + WorldHelper.getMeta(world, x, y, z)
                    + ". ";
            }
            if (isBaseBlockPresent(world, x, y, z)) {
                ...
                txt += "Base block (" + bedrock
                    + ") is MISSING. Block directly below soil is "
                    + WorldHelper.getBlock(world, x, y - 1, z)
                    + ":"
                    + WorldHelper.getMeta(world, x, y - 1, z)
                    + ", block below that is "
                    + WorldHelper.getBlock(world, x, y - 2, z)
                    + ":"
            ...
            if (isBrightnessOk(world, x, y, z)) {
                txt += "Light levels (sky light " + brightness[2]
                    + "..."
                    + brightness[3]
                    + " and block light "
                    + brightness[0]
        ...
        return isValidSoil(world, x, y - 1, z)
            && (isBaseBlockPresent(world, x, y, z) || isBaseBlockPresent(world, x, y - 1, z))
            && areBarsPresent(world, x, y, z)
            && isBrightnessOk(world, x, y, z)
    ...
    private static boolean isBlock(World world, int x, int y, int z, BlockWithMeta bwm) {
        return WorldHelper.getBlock(world, x, y, z) == bwm.getBlock()
            && (bwm.ignoreMeta() || WorldHelper.getMeta(world, x, y, z) == bwm.getMeta());
    }
    ...
    public boolean isBrightnessOk(World world, int x, int y, int z) {
        ...
        return skyLight >= brightness[2] && skyLight <= brightness[3]
            && blockLight >= brightness[0]
            && blockLight <= brightness[1];
    }
