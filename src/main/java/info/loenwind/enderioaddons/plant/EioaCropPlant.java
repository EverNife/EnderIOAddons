...
public class EioaCropPlant implements ICropPlant {
    ...
    public ItemStack getRandomFruit(Random rand) {
        return ((WeightedItemStack) WeightedRandom
            .getRandomItem(rand, ItemHelper.readWeightedList(Config.plantDropsHighGain))).getStack()
                .copy();
    }
    ...
    public ArrayList<ItemStack> getFruitsOnHarvest(int gain, Random rand) {
        ...
        for (int i = 0; i < (gain > 2 ? 2 : 1); i++) {
            result.add(
                ((WeightedItemStack) WeightedRandom
                    .getRandomItem(rand, ItemHelper.readWeightedList(Config.plantDropsLowGain))).getStack()
                        .copy());
        }
        if (gain > 5) {
    ...
    public void onSeedPlanted(World world, int x, int y, int z) {
        PacketHandler.INSTANCE.sendToAllAround(
            new PacketFarmAction(new BlockCoord(x, y, z)),
